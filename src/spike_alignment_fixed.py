...
def diagnose_d1_alignment(d1_path):
    """
...
def compare_extraction_methods(d1_path):
    """
...
def prepare_aligned_training_data(d1_path, align_peak_at=30, verbose=True):
    """
...
def main():
    ...
    # Try to find D1.mat
    possible_paths = [
        Path('datasets/D1.mat'),
        Path('../datasets/D1.mat'),
        Path('D1.mat'),
        Path('/mnt/user-data/uploads/D1.mat'),
    ]
    ...
    if d1_path is None:
        print("ERROR: Could not find D1.mat")
        print("Please ensure D1.mat is in one of these locations:")
        for p in possible_paths:
            print(f"  - {p}")
        return
    
    print(f"Found D1.mat at: {d1_path}")
    
    # Run diagnostics
    stats = diagnose_d1_alignment(d1_path)
    compare_extraction_methods(d1_path)
    
    # Prepare aligned training data
    waveforms, classes = prepare_aligned_training_data(d1_path, align_peak_at=30)
